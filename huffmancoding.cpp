//HELLO,I AM ARITRA RAY......I BUILD A PROJECT ON TEXT COMPRESSION-DECOMPRESSION WITH THE HELP OF HUFFMAN ALGORITHM

//THE C++ code of this project is here--->
#include <iostream>
#include<bits/stdc++.h>
using namespace std;
...
//Creating a class for HuffmanCode most of the functions including compression and decompression reside here
...
private:
    ...
    void __Build_Binary_Code_Healper(BinaryTree* root, string curr_bits) {
        if (root == nullptr) {
            return;
        }
        if (root->value != '\0') {
        ...
        __Build_Binary_Code_Healper(root->left, curr_bits + "0");
        __Build_Binary_Code_Healper(root->right, curr_bits + "1");
    }

    //Previous and next both functions are designed to assign Binary code to the characters
    void __Build_Binary_Code() {
        BinaryTree* root = heap.front();
        __Build_Binary_Code_Healper(root, "");
    }
...
public:
    ...
    string compression() {
        ...
        cout << "COMPRESSION OF YOUR FILE STARTS..." << endl;
        ...
        cout << "COMPESSED SUCCESSFULLY." << endl;
        return output_path;
    }
...
int main() {
    ...
    cout << "ENTER THE PATH OF YOUR FILE WHICH YOU NEED TO COMPRESS: ";
    getline(cin, path);
    Huffmancode h(path);
